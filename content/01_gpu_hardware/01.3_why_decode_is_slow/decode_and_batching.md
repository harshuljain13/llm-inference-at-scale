...
**Q1: Why is decode so much slower than prefill?**

Prefill processes all input tokens simultaneously as a large matrix-matrix multiplication (high arithmetic intensity, compute-bound). Decode generates one token at a time per sequence, performing matrix-vector multiplications that read the entire model from HBM for minimal compute. The arithmetic intensity drops from hundreds of FLOPs/byte (prefill) to approximately 1 FLOP/byte (single-token decode), meaning the GPU spends 99% of its time waiting for memory transfers rather than computing.

**Q2: Does batching increase latency for individual requests?**

Batching adds minimal latency to individual requests as long as VRAM is not exhausted. The model weights are read once regardless of batch size, so the memory transfer cost is amortized. The additional compute per sequence is tiny relative to the memory stall. However, once the batch becomes large enough that KV cache spills to slower memory or scheduling delays queue requests, per-request latency increases. In practice, latency remains nearly flat up to the point where VRAM pressure forces eviction or recomputation.

**Q3: Can I ever make decode compute-bound?**

Only in narrow regimes. With very short context lengths (under 128 tokens), KV cache reads are small enough that batch sizes of 183+ (A100) or 433+ (H100) can theoretically push arithmetic intensity past the ridge point. For typical generative workloads with context lengths above 2K tokens, the KV cache bandwidth cost per additional sequence exceeds the FLOPs gained, making it mathematically impossible to reach the ridge point regardless of batch size.

**Q4: Why do KV cache reads create diminishing returns for batching?**

Each sequence in the batch brings its own KV cache that must be read from HBM during attention computation. As batch size grows, KV cache bytes grow linearly with B in the denominator of the arithmetic intensity formula. The numerator (FLOPs) also grows linearly with B, but the denominator grows as (fixed_model_bytes + B * kv_bytes), so the ratio saturates. At long contexts, kv_bytes_per_sequence is large enough that adding sequences barely improves AI.

**Q5: If H100 has 1.64x the bandwidth of A100, why is decode only 1.64x faster and not 3.2x?**

Decode is memory-bound, so throughput scales with memory bandwidth, not compute. The H100 has 3.2x the FLOPs of A100 but only 1.64x the bandwidth (3,352 vs 2,039 GB/s). Since the bottleneck is streaming model weights from HBM, the bandwidth ratio accurately predicts the speedup. The extra compute capacity of H100 goes unused during decode because the execution units are starved for data.

**Q6: What is the most effective way to improve decode throughput beyond batching?**

Three complementary strategies: (1) Quantization reduces model bytes, increasing both the maximum achievable tokens/second (bandwidth / smaller model) and arithmetic intensity at every batch size. INT8 doubles throughput ceiling; INT4 quadruples it. (2) GQA/MQA reduces KV cache size per sequence, slowing the growth of the denominator and allowing larger effective batch sizes within fixed VRAM. (3) Speculative decoding generates multiple tokens per forward pass, amortizing the weight-read cost over several tokens instead of one, effectively multiplying the numerator without proportionally increasing memory reads.
...
2. NVIDIA. (2023). H100 Tensor Core GPU Architecture Whitepaper.
3. NVIDIA. (2024). B200 GPU Datasheet.
4. Pope, R., et al. (2022). Efficiently Scaling Transformer Inference. MLSys.
5. Sheng, Y., et al. (2023). FlexGen: High-Throughput Generative Inference of Large Language Models with a Single GPU. ICML.
6. Kwon, W., et al. (2023). Efficient Memory Management for Large Language Model Serving with PagedAttention. SOSP.
