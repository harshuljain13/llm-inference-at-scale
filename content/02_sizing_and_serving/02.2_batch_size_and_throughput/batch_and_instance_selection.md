...
**Q1: Why not just maximize batch size to get the highest throughput?**

Larger batch sizes increase KV cache memory linearly while weights remain constant. Beyond the crossover point (where KV cache reads exceed weight reads), each additional sequence adds measurable per-token latency. At batch size 96 on an A100 80GB with Llama 8B, you hit 84% VRAM utilization with zero headroom for prefill spikes. Maximizing batch size trades latency and stability for throughput, which violates SLAs in latency-sensitive applications.

**Q2: When should I use 2 smaller GPUs instead of 1 larger one?**

Use multiple smaller GPUs when: (1) the model does not fit on a single GPU at your target precision, (2) you need more aggregate HBM bandwidth to meet latency SLAs, or (3) cost-per-token is lower with the multi-GPU configuration after accounting for tensor-parallel communication overhead. A single larger GPU avoids NVLink/PCIe communication costs entirely, so prefer it when the model fits and bandwidth is sufficient.

**Q3: How do I choose between quantization and adding more GPUs?**

Start with INT8 quantization (AWQ or GPTQ) and validate quality on your specific task. INT8 halves memory and doubles effective bandwidth with typically less than 1% quality degradation. If quality drops unacceptably (common for reasoning-heavy or code tasks with INT4), scale to more GPUs at FP16/FP8 instead. The decision is: quantization gives 2-4x cost reduction but risks quality; more GPUs preserve quality but multiply cost linearly.

**Q4: How much VRAM headroom should I leave beyond the calculated budget?**

Leave at least 5-10% of total VRAM unallocated. CUDA context consumes 1.5-3 GB at startup, memory fragmentation wastes 5-15%, and prefill activation spikes for long sequences can temporarily require several GB. A model that calculates to 78 GB on an 80 GB GPU will likely OOM during load or under peak prefill. Target 85-90% peak utilization as the practical ceiling.

**Q5: Does increasing context length or batch size hurt performance more?**

Both increase KV cache linearly, but their downstream effects differ. Doubling context length doubles KV per sequence and also increases prefill compute quadratically (for standard attention). Doubling batch size doubles total KV but prefill compute scales only linearly (each new sequence is independent). In the decode phase, both have equivalent memory-bandwidth impact. In practice, long-context workloads are harder to serve because they combine decode memory pressure with expensive prefill spikes.

**Q6: When does it make sense to use inference-optimized GPUs (L4, A10G) vs. training GPUs (A100, H100)?**

Inference GPUs (L4, A10G) are cost-effective for models that fit in 24-48 GB VRAM (7B-13B quantized). They lack the HBM bandwidth of A100/H100, so per-token latency is higher, but cost-per-token can be 3-5x lower for throughput-oriented workloads. Use training-class GPUs when: the model requires more than 48 GB VRAM, latency SLAs demand high bandwidth (sub-20ms per token), or you need multi-GPU tensor parallelism with NVSwitch interconnects.
